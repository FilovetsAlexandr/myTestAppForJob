...
class PhotosListViewController: UIViewController {
    ...
    private var items: [PhotosListTableCellModel] = [] { didSet { tableView.reloadData() } }
    ...
    private func render(state: PhotosListViewModel.State) {
        ...
        case .loaded(let items, let isLastPage, let message):
            ...
            self.items = items
            if isLastPage { self.tableView.tableFooterView = nil }
            if let message { showAlert(title: "Success", message: message) }
        case .showingCamera:
            showCamera()
